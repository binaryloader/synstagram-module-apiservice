...
extension AuthService {
    ...
    public func requestIsAlreadyLogin(completion: @escaping LoginCompletion) {
        ...
        provider.request(target: target) { result in
            ...
            case .success(let response):
                guard let loginDTO = try? JSONDecoder().decode(
                    AuthLoginModel.Response.DTO.self,
                    from: response.data
                ) else {
    ...
    public func requestLogin(
        request: AuthLoginModel.Request,
...
extension AuthService {
    ...
    public func requestLogout(completion: @escaping LogoutCompletion) {
        ...
        provider.request(target: target) { result in
            switch result {
            case .success(let response):
                guard let logoutDTO = try? JSONDecoder().decode(
                    AuthLogoutModel.Response.DTO.self,
                    from: response.data
                ) else {
